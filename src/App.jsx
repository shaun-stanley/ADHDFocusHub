...
import { Brain, CheckCircle2, Clock, Coffee, Focus, ListChecks, Plus, Rocket, Settings, Star, Target, Trash2, Volume2, X, Flame, Gift, Play, Pause, RefreshCcw, Maximize2, Minimize2, Bug } from "lucide-react";
...
const useWhiteNoise = () => { const ctxRef = useRef(null); const noiseRef = useRef(null); const [on, setOn] = useState(false);
  const toggle = async () => { if (on) { try { noiseRef.current?.stop?.(); } catch {}; noiseRef.current = null; setOn(false); return; }
    try { const Ctx = window.AudioContext || window.webkitAudioContext; const ctx = ctxRef.current || new Ctx(); ctxRef.current = ctx; if (ctx.state === "suspended") await ctx.resume();
      ...
      const whiteNoise = ctx.createBufferSource(); whiteNoise.buffer = noiseBuffer; const filter = ctx.createBiquadFilter(); filter.type = "lowpass"; filter.frequency.value=1000; whiteNoise.connect(filter); filter.connect(ctx.destination); whiteNoise.loop = true; whiteNoise.start(); noiseRef.current = whiteNoise; setOn(true);
    } catch {} };
...
export default function ADHDFocusHub(){
  ...
  return (<div className="min-h-screen bg-gradient-to-br from-sky-50 via-white to-violet-50 text-slate-800">
    <header className="sticky top-0 z-30 backdrop-blur supports-[backdrop-filter]:bg-white/60 bg-white/80 border-b">
      <div className="max-w-6xl mx-auto px-3 md:px-4 py-3 flex items-center gap-3">
        ...
        <h1 className="text-xl font-semibold leading-tight">ADHD <span className="hidden xs:inline">Focus</span> Hub</h1>
        <Badge className="ml-1" variant="secondary">Beta</Badge>
        <div className="ml-auto flex flex-wrap items-center gap-2 sm:gap-3">
          <div className="sm:hidden"><Button variant="ghost" size="icon" aria-label="Focus mode" onClick={()=>setFocusMode(true)}><Focus className="w-4 h-4" /></Button></div>
          <div className="hidden sm:block"><Button variant="ghost" size="sm" onClick={()=>setFocusMode(true)}><Focus className="w-4 h-4 mr-1" /> Focus mode</Button></div>
          <div className="sm:hidden"><Button variant={whiteNoiseOn?'default':'secondary'} size="icon" aria-label="Toggle white noise" onClick={toggleNoise}><Volume2 className="w-4 h-4" /></Button></div>
          <div className="hidden sm:block"><Button variant={whiteNoiseOn?'default':'secondary'} size="sm" onClick={toggleNoise}><Volume2 className="w-4 h-4 mr-1" /> {whiteNoiseOn?'White noise on':'White noise off'}</Button></div>
          <Button variant="ghost" size="icon" onClick={()=>setFullscreen(f=>!f)} title="Toggle fullscreen">{fullscreen?<Minimize2 className="w-5 h-5" />:<Maximize2 className="w-5 h-5" />}</Button>
          ...
          <CardContent className="space-y-3">
            <div className="flex flex-col md:flex-row gap-2">
              ...
              <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
                <select className="w-full sm:w-36 border rounded-xl px-2 py-2" value={priority} onChange={(e)=>setPriority(e.target.value)}>
                  <option value="H">High</option><option value="M">Medium</option><option value="L">Low</option>
                </select>
                <div className="flex items-center gap-2 w-full sm:w-auto">
                  <Clock className="w-4 h-4 shrink-0"/><Input type="number" className="w-full sm:w-20" value={eta} onChange={(e)=>setEta(Number(e.target.value||15))}/><span className="text-sm text-slate-500 shrink-0">min</span>
                </div>
                <Button size="sm" fluid className="sm:w-auto" onClick={addTask}><Plus className="w-4 h-4 mr-1"/>Add</Button>
              </div>
            </div>
                ...
                <motion.div key={t.id} layout className="border rounded-xl p-3 flex items-start gap-3 bg-white">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <Badge variant={t.priority==='H'?'default':t.priority==='M'?'secondary':'outline'}>{t.priority}</Badge>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <Button size="icon" variant="secondary" title="Focus on this"><Target className="w-4 h-4"/></Button>
                  </div>
                </motion.div>
              ))}
            </div>
        ...
        <Card className="shadow-sm">
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              <div><p className="text-sm text-slate-500">{onBreak ? "Break" : "Focus"} time</p><div className="text-4xl font-bold tabular-nums">{niceTime(remaining)}</div></div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div><p className="text-xs text-slate-500 mb-1">Focus minutes: {Math.round(focusSec/60)}</p><Slider value={[Math.round(focusSec/60)]} onValueChange={(v)=>setFocusSec(v[0]*60)} min={5} max={90}/></div>
              <div><p className="text-xs text-slate-500 mb-1">Break minutes: {Math.round(breakSec/60)}</p><Slider value={[Math.round(breakSec/60)]} onValueChange={(v)=>setBreakSec(v[0]*60)} min={3} max={30}/></div>
            </div>
            <div className="mt-3 text-xs sm:text-sm text-slate-500 flex items-center gap-2"><Flame className="w-4 h-4"/> Pomodoros completed today: <span className="font-semibold">{completedPomos}</span></div>
          </CardContent>
        </Card>
              ...
              {habits.map((h)=>(
                <div key={h.id} className="flex items-center justify-between border rounded-xl p-3 bg-white">
                  <div className="flex items-center gap-3">
                    ...
                    <div><div className="font-medium">{h.name}</div><div className="text-xs text-slate-500">Streak: {h.streak} 🔥</div></div>
                  </div>
    ...
    <Dialog open={focusMode} onOpenChange={setFocusMode}>
      <DialogContent className="max-w-2xl">
        ...
        <div className="space-y-4">
          ...
          <FocusPanel task={tasks.find(t=>t.id===selectedTaskId)} onClose={()=>setFocusMode(false)} onStart={()=>setIsRunning(true)} onPause={()=>setIsRunning(false)} remaining={remaining} onBreak={onBreak} niceTime={niceTime} />
        </div>
...
function FocusPanel({ task, onClose, onStart, onPause, remaining, onBreak, niceTime }){
  return (<div className="rounded-2xl bg-gradient-to-br from-indigo-50 to-emerald-50 p-6">
    <div className="flex items-start justify-between">
      <div className="min-w-0">
        ...
        {task && (<div className="mt-2 flex items-center gap-2 flex-wrap"><Badge variant={task.priority==='H'?'default':task.priority==='M'?'secondary':'outline'}>{task.priority}</Badge><Badge variant="outline">~{task.eta}m</Badge></div>)}
      </div>
    ...
    <div className="mt-6 grid gap-6 sm:grid-cols-2">
      <div className="border rounded-2xl p-6 bg-white flex flex-col items-center justify-center">
        ...
        <div className="flex items-center gap-2 w-full"><Button fluid onClick={onStart}><Play className="w-4 h-4 mr-1"/>Start</Button><Button fluid variant="secondary" onClick={onPause}><Pause className="w-4 h-4 mr-1"/>Pause</Button></div>
      </div>
...
function DevTests(){ const [open,setOpen]=useState(false); const tests=useMemo(()=>runSelfTests(),[]); const passed=tests.every(t=>t.pass); return (<div className="max-w-6xl mx-auto px-3 md:px-4 my-6"><button onClick={()=>setOpen(o=>!o)} className="text-xs inline-flex items-center gap-2 px-3 py-1.5 rounded-full border bg-white hover:bg-slate-50"><Bug className="w-3.5 h-3.5"/> {open?"Hide":"Show"} self-tests {passed?"✅":"⚠️"}</button>{open&&(<div className="mt-3 grid gap-2">{tests.map((t,i)=>(<div key={i} className={`text-xs p-2 rounded border ${t.pass?"bg-emerald-50 border-emerald-200":"bg-rose-50 border-rose-200"}`}><div className="font-medium">{t.name}</div><div>{t.pass?"PASS":"FAIL"}</div></div>))}</div>)}</div>); }
