...
export function Dialog({open,onOpenChange,children}){const ref=useRef();useEffect(()=>{if(!ref.current)return;if(open)ref.current.showModal?.();else ref.current.close?.();},[open]);return <dialog ref={ref} className='rounded-2xl p-0 backdrop:bg-black/40 w-[92vw] max-w-xl' onClose={()=>onOpenChange?.(false)}>{children}</dialog>}
